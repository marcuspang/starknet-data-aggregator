...
@external
func get_24_hour_wallets_interacted{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(address: felt) -> (tx_len: felt, tx: WalletTransaction*) {
    let (total_length) = nft_transaction_data_length.read();
...
func get_24_hour_wallets_interacted_internal{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    ...
    assert wallets[current_index] = WalletTransaction(
        address=nft_tx.buyer_address,
        tx_hash=nft_tx.tx_hash,
        type=nft_tx.event_type,
        );

    let (wallets_len, wallets) = get_24_hour_wallets_interacted_internal(
        current_index + 1, total_length, wallets_len + 1, wallets
    );
